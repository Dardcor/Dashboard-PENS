          ...
          <table className="table">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={mhs.id}>
                  <td className="text-muted" style={{ fontWeight: 500 }}>{mhs.nrp}</td>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <div style={{ flex: 1, height: '6px', backgroundColor: 'var(--color-border)', borderRadius: '3px', overflow: 'hidden' }}>
                        <div style={{ 
                          ...
                          height: '100%', 
                          backgroundColor: mhs.avg_kehadiran >= 80 ? 'var(--color-success)' : 'var(--color-danger)' 
                        }}></div>
                      </div>
                      <span style={{ fontSize: '0.75rem', fontWeight: 600 }}>{mhs.avg_kehadiran || 0}%</span>
                    </div>
                  </td>
                  <td>
                    <span className={`badge ${(mhs.ipk_kumulatif || 0) >= 3.0 ? 'badge-success' : 'badge-warning'}`}>
                  ...
                  <td>
                    {mhs.jumlah_alert_aktif > 0 ? (
                      ...
                      <span className="text-muted" style={{ fontSize: '0.75rem' }}>Aman</span>
                    )}
                  </td>
                  <td>
                    <Link to={`/dosen/mahasiswa/${mhs.id}`} className="btn btn-secondary" style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem' }}>
                      <Eye size={14} /> Detail
                    </Link>
                  </td>
                </tr>
