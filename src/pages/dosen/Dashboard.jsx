...
export default function DosenDashboard() {
  ...
  const statCards = [
    ...
    {
      title: 'IPK < 3.0',
      value: mahasiswa.filter(m => m.ipk_kumulatif < 3.0).length,
      icon: TrendingDown,
      color: 'var(--color-warning)'
    },
    {
      title: 'Kehadiran < 80%',
      value: mahasiswa.filter(m => m.avg_kehadiran < 80).length,
      icon: Clock,
      color: 'var(--color-danger)'
    }
  ];
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="card" style={{ gridColumn: 'span 2' }}>
          <div className="card-header">
            ...
            <button className="btn btn-secondary" style={{ fontSize: '0.75rem', padding: '0.25rem 0.5rem' }}>Lihat Semua</button>
          </div>
              ...
              <tbody>
                {alerts.slice(0, 5).map(alert => (
                  <tr key={alert.id}>
                    ...
                    <td>
                      <span className={`badge ${alert.tipe_alert === 'kehadiran_buruk' ? 'badge-danger' : alert.tipe_alert === 'ipk_turun' ? 'badge-warning' : 'badge-primary'}`}>
                        {alert.tipe_alert.replace('_', ' ').toUpperCase()}
                      </span>
                    </td>
                    <td>
                      <span className="text-danger" style={{ fontSize: '0.875rem', fontWeight: 500 }}>Butuh Perhatian</span>
                    </td>
                  </tr>
                ))}
                {alerts.length === 0 && (
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {mahasiswa.filter(m => parseInt(m.jumlah_alert_aktif) > 0).map(mhs => (
              <div key={mhs.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingBottom: '1rem', borderBottom: '1px solid var(--color-border)' }}>
                ...
                <button className="btn btn-primary" style={{ padding: '0.25rem 0.75rem', fontSize: '0.75rem' }}>Jadwalkan</button>
              </div>
            ))}
            {mahasiswa.filter(m => parseInt(m.jumlah_alert_aktif) > 0).length === 0 && (
              <p className="text-muted" style={{ fontSize: '0.875rem', textAlign: 'center' }}>Tidak ada mahasiswa yang perlu konsultasi mendesak.</p>
            )}
          </div>
        </div>
      </div>
