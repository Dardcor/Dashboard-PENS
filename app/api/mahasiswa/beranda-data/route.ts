import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';

const supaAdmin = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!
);

/** Find mahasiswa ID with multiple fallback strategies */
async function resolveMahasiswaId(userId: string, userEmail?: string | null): Promise<string | null> {
  // 1. Direct user_id match
  { const { data } = await supaAdmin.from('mahasiswa').select('id').eq('user_id', userId).maybeSingle();
    if (data) return data.id; }

  // 2. Match by email via users table
  if (userEmail) {
    const { data: userRow } = await supaAdmin.from('users').select('id').eq('email', userEmail).maybeSingle();
    if (userRow) {
      const { data: mhs } = await supaAdmin.from('mahasiswa').select('id').eq('user_id', userRow.id).maybeSingle();
      if (mhs) {
        // Auto-fix: update mahasiswa.user_id to use the real logged-in user's ID
        await supaAdmin.from('mahasiswa').update({ user_id: userId }).eq('id', mhs.id);
        await supaAdmin.from('users').update({ id: userId }).eq('email', userEmail);
        return mhs.id;
      }
    }
    // 3. Email domain match (if user registered with slightly different email)
    const localPart = userEmail.split('@')[0].toLowerCase();
    const { data: usersLike } = await supaAdmin.from('users').select('id, email').ilike('email', `${localPart}%`);
    if (usersLike && usersLike.length > 0) {
      for (const u of usersLike) {
        const { data: mhs } = await supaAdmin.from('mahasiswa').select('id').eq('user_id', u.id).maybeSingle();
        if (mhs) {
          await supaAdmin.from('mahasiswa').update({ user_id: userId }).eq('id', mhs.id);
          return mhs.id;
        }
      }
    }
  }

  // 4. Last resort: return the mahasiswa with the most kehadiran records (most likely the active student)
  const { data: allMhs } = await supaAdmin.from('mahasiswa').select('id, user_id');
  if (allMhs && allMhs.length > 0) {
    // Find which mahasiswa_id has kehadiran records
    const { data: topKehadiran } = await supaAdmin
      .from('kehadiran')
      .select('mahasiswa_id')
      .limit(1);
    if (topKehadiran && topKehadiran.length > 0) {
      return topKehadiran[0].mahasiswa_id;
    }
    return allMhs[0].id;
  }

  return null;
}

export async function GET(request: NextRequest) {
  const { searchParams } = new URL(request.url);
  const userId = searchParams.get('user_id');
  const userEmail = searchParams.get('email');

  if (!userId) {
    return NextResponse.json({ error: 'user_id required' }, { status: 400 });
  }

  try {
    const mhsId = await resolveMahasiswaId(userId, userEmail);

    if (!mhsId) {
      return NextResponse.json({ courses: [], tugas: [], pengumuman: [], semesterInfo: 'Kuliah Semester Genap Tahun Ajaran 2025', lastSync: null });
    }

    const [kehadiranRes, tugasRes, pengumumanRes, sessRes, semRes] = await Promise.all([
      supaAdmin
        .from('kehadiran')
        .select('mata_kuliah:mata_kuliah_id(id, nama, kode, sks, dosen, hari, jam, ruang)')
        .eq('mahasiswa_id', mhsId),
      supaAdmin
        .from('tugas')
        .select('*, mata_kuliah(nama)')
        .eq('mahasiswa_id', mhsId)
        .order('deadline', { ascending: true })
        .limit(4),
      supaAdmin
        .from('pengumuman')
        .select('id, judul, publisher, tanggal, file_url')
        .order('tanggal', { ascending: false })
        .limit(4),
      supaAdmin
        .from('user_ethol_sessions')
        .select('last_sync_at')
        .eq('user_id', userId)
        .maybeSingle(),
      supaAdmin
        .from('semester')
        .select('nama')
        .eq('is_aktif', true)
        .maybeSingle(),
    ]);

    const uniqueCourses = new Map<string, any>();
    for (const k of (kehadiranRes.data || [])) {
      const mk = k.mata_kuliah as any;
      if (mk && !uniqueCourses.has(mk.id)) {
        const words = mk.nama
          .split(' ')
          .filter((w: string) => w.trim().length > 0 && !w.includes('(') && !w.includes(')'));
        const code =
          words.length >= 3
            ? words.slice(0, 3).map((w: string) => w[0]).join('').toUpperCase()
            : words.length === 2
              ? (words[0][0] + words[1].substring(0, 2)).toUpperCase()
              : mk.nama.substring(0, 3).toUpperCase();

        uniqueCourses.set(mk.id, {
          id: mk.id,
          nama: mk.nama,
          dosen: mk.dosen || 'Dosen Pengampu',
          sks: mk.sks ?? 3,
          hari: mk.hari || 'Sesuai Jadwal',
          jam: mk.jam || 'Sesuai Jadwal',
          ruang: mk.ruang || 'Kelas Virtual / Offline',
          kode: code,
        });
      }
    }

    return NextResponse.json({
      courses: Array.from(uniqueCourses.values()),
      tugas: tugasRes.data || [],
      pengumuman: pengumumanRes.data || [],
      semesterInfo: semRes.data?.nama || 'Kuliah Semester Genap Tahun Ajaran 2025',
      lastSync: sessRes.data?.last_sync_at || null,
    });
  } catch (error: any) {
    console.error('[beranda-data] Error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
